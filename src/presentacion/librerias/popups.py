...
class PopUp(Sprite):
    def __init__(self, parent , texto1, palabra_boton , imagenes , grupo, tipo = 0 ,px=512,py=281,tam =0):
        ...
        if tipo == 0: 
            ...
            self.area_texto = Rect(x,y,self.sprite.rect.w*2/3,self.texto.ancho_final)   
            self.area_imagenes = Rect((self.sprite.rect.w*2/3)+80, y, self.sprite.rect.w/3, self.texto.ancho_final)           
            self.parent = parent
            self.boton = TextButton(0,self.parent,palabra_boton)            
            self.boton.relocate(self.sprite.rect.width/2, self.area_texto.h + x*2 + self.boton.rect.h / 2)
            self.boton_rect = Rect(self.boton.rect.x  ,  self.boton.rect.y  , self.boton.rect.width  ,  self.boton.rect.height) 
        ...
        if tipo == 1:
            ...
            for i in texto1:
                ...
                if o>0:
                    self.arreglo_botones.append(TextButton(o-1,self.parent,i,1,self.sprite.rect.w - x*2 -tabulacion))
                o+=1        
            self.texto.rect = Rect(x,y,self.sprite.rect.w - 80, self.texto.ancho_final)                                
